...
{
	wchar_t const      *base_name;
	wchar_t const      *sig_name;
...
rs_result rdiff_sig(const wchar_t *baseFile, const wchar_t *sigFile)
{
	...
	basis_file = rs_file_open(baseFile, L"rb");
	sig_file = rs_file_open(sigFile, L"wb");
...
static rs_result rdiff_delta_internal(poptContext opcon)
{
	...
	wchar_t const      *sig_name, *new_name, *delta_name;
	rs_result       result;
	...
	new_name = poptGetArg(opcon);
	delta_name = poptGetArg(opcon);
...
rs_result rdiff_delta(const wchar_t *sigFile, const wchar_t *newFile, const wchar_t *deltaFile)
{
	FILE            *sig_file, *new_file, *delta_file;
	rs_result       result;
	rs_signature_t  *sumset;
	...
	sig_file = rs_file_open(sigFile, L"rb");
	new_file = rs_file_open(newFile, L"rb");
	delta_file = rs_file_open(deltaFile, L"wb");
...
static rs_result rdiff_patch_internal(poptContext opcon)
{
	...
	wchar_t const         *basis_name, *delta_name, *new_name;
...
rs_result rdiff_patch(const wchar_t *baseFile, const wchar_t *deltaFile, const wchar_t *newFile)
{
	/*  patch BASIS [DELTA [NEWFILE]] */
	...
	basis_file = rs_file_open(baseFile, L"rb");
	delta_file = rs_file_open(deltaFile, L"rb");
	new_file =   rs_file_open(newFile, L"wb");
...
static rs_result rdiff_action(poptContext opcon)
{
	...
	if (!action)
		...
		return rdiff_sig_internal(opcon);
	else if (isprefix(action, "delta"))
		return rdiff_delta_internal(opcon);
	else if (isprefix(action, "patch"))
		return rdiff_patch_internal(opcon);
